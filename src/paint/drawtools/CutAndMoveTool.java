...
/**
 * Tool to cut and paste.
 * 
 * @author ericyager
 */
public class CutAndMoveTool extends DrawTool {

    private double startx;
    private double starty;
    private Image initialCanvas;
    private Image intermediaryCanvas;
    private boolean imageSelected = false;
    private Image selectedImage;
    private double imagex;
    private double imagey;

    public CutAndMoveTool() {
        super();
    }

    public CutAndMoveTool(Canvas canvas) {
        super(canvas);
    }
    ...
    public void onMouseDragged(MouseEvent e) {
        if (!imageSelected) {
            ...
            super.getCanvas().getGraphicsContext2D().drawImage(intermediaryCanvas, 0, 0);
    ...
    public void onMouseReleased(MouseEvent e) {
        if (!imageSelected) {
            super.getCanvas().getGraphicsContext2D().drawImage(initialCanvas, 0, 0);
            Paint tempColor = super.getCanvas().getGraphicsContext2D().getFill();
            super.getCanvas().getGraphicsContext2D().setFill(Color.WHITE);
            if (e.getX() < startx && e.getY() < starty) { //If the user dragged left after clicking
            ...
            intermediaryCanvas = ImageTransformer.canvasToImage(super.getCanvas());
            imageSelected = true;
            super.getCanvas().getGraphicsContext2D().drawImage(selectedImage, imagex, imagey);
            drawOutline(imagex, imagey, selectedImage.getWidth(), selectedImage.getHeight());
        } else {
